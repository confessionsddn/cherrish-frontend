...
const MoodVideoLoop = ({ moodName }) => {
  ...
  // Filename logic: "Secret Admirer" -> "secretadmirer.mp4"
  const fileName = `${moodName.toLowerCase().replace(/\s+/g, '')}.mp4`
...
export default function MoodSelector({ selectedMood, onMoodChange }) {
  ...
  // --- Animation Logic (kept exactly as provided) ---
  const triggerHeartbreakTransition = () => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => { overlay.style.display = 'none'; overlay.innerHTML = '' }, 600)
    }, 4000)
  }
